  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-[#2a2a2a] pb-4">
        <h2 className="text-xl font-bold text-[#f2f2f2]">{title}</h2>
        <div className="flex gap-2">
          <Button
            variant="outline"
            size="sm"
            className="bg-[#1c1c1c] border-[#2a2a2a] text-[#f2f2f2] h-8 text-xs hover:bg-[#2a2a2a] hover:text-white"
          >
      ...
      {videos.length === 0 && !isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-[#2a2a2a] rounded-xl bg-[#121212]/50">
          ...
          <p className="text-[#a1a1a1]">
            No assets ingested yet. Start a crawl or check progress to see videos here.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          ...
          {isLoading &&
            Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="aspect-[9/16] bg-[#1c1c1c] animate-pulse rounded-xl border border-[#2a2a2a]"
              />
            ))}
        </div>
      )}
